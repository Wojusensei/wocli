...
2026.08.22 v0.5.1：修复 badapple 播放上下抖动——灰度改用 ANSI 背景色画空格（块字符是 Unicode 宽度模糊字符，中文环境终端常渲染成两列宽导致整行折行错位）；帧尾光标回卷避开末行待换行滚动；全部帧预渲染为 bytes 播放零计算；进入备用屏幕缓冲区（Terminal.app 优化路径）；新增 ```WOCLI_BADAPPLE_FPS``` 环境变量可降帧换稳定

2026.08.22 v0.5.2：修复 ```wocli qr``` 生成的二维码无法被微信等识别——静区宽度 border=2 违反 ISO/IEC 18004 标准（须 >=4 模块），改为标准值；纠错级别 L 提升至 M；badapple 在 Windows 上零依赖配乐（探测不到播放器时通过 PowerShell 调用系统自带 WMP）
